...
const Hint = ({ children }: { children: React.ReactNode }) => (
  <span className="rounded-md border border-ink/10 bg-ink/5 px-1.5 py-0.5 text-ink/50">
    {children}
  </span>
);
...
const CommandMenu = () => {
  ...
  const run = useCallback(
    (command: ICommand) => {
      ...
      if (command.id === "theme") {
        // No click to grow from here, so the sweep starts at the middle.
        toggleTheme();
        return;
      }
    ...
    [close, copyLink, reduceMotion, router, theme, toggleTheme],
  );
      ...
      {open && (
        <div className="fixed inset-0 z-[60] flex items-start justify-center px-4 pb-4 pt-[10vh]">
          ...
          <motion.div
            ...
            className="relative flex max-h-full w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-ink/10 bg-panel shadow-[0_24px_70px_-12px_var(--shadow)]"
            initial={
          ...
          >
            <div className="flex items-start gap-3.5 border-b border-ink/10 px-4 py-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-ink/10 bg-ink/5 text-ink/70">
              ...
              <button
                ...
                className="ml-auto shrink-0 rounded-lg p-1 text-ink/40 transition-colors hover:bg-ink/10 hover:text-ink"
              >
            ...
            <div className="flex items-center gap-3 border-b border-ink/10 px-4 py-2.5">
              ...
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="w-full bg-transparent text-sm font-light text-ink outline-none placeholder:text-ink/35"
              />
            </div>
            ...
            >
              {flat.length === 0 && (
                <p className="px-5 py-8 text-center text-sm font-light text-ink/40">
                  No actions found.
                </p>
              )}
              {groups.map((group) => (
                <div key={group.heading} className="px-2 pb-0.5">
                  <p className="px-3 py-1.5 text-xs font-light tracking-wide text-ink/35">
                    {group.heading}
                  </p>
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors ${
                          isActive ? "bg-ink/10 text-ink" : "text-ink/70"
                        }`}
                      >
                        <Icon size={17} className="shrink-0 text-ink/55" />
                        ...
                        {copied && item.id === "copy-link" ? (
                          ...
                          item.shortcut && (
                            <span className="rounded-md border border-ink/10 bg-ink/5 px-2 py-1 text-[11px] font-light text-ink/50">
                              shift + {item.shortcut}
                            </span>
                          )
                        )}
            ...
            <div className="flex items-center justify-between gap-4 border-t border-ink/10 px-4 py-2.5 text-[11px] font-light text-ink/35">
